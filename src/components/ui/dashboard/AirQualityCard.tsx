...
const AirQualityCard: React.FC<AirQualityCardProps> = ({
  ...
  const getStatusClass = (status?: 'good' | 'moderate' | 'unhealthy') => {
    ...
    if (status === 'moderate') return 'bg-yellow-100 text-yellow-800 dark:bg-yellow-900/30 dark:text-yellow-400';
    if (status === 'unhealthy') return 'bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-400';
          ...
          <div className="space-y-4 w-full max-w-md">
            <div className="text-center md:text-left">
              <span className={cn(
                ...
                index <= 50 ? 'bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-400' :
                index <= 100 ? 'bg-yellow-100 text-yellow-800 dark:bg-yellow-900/30 dark:text-yellow-400' :
              ...
              {indicators.map((indicator, i) => (
                <div key={i} className="space-y-1">
                  <div className="flex justify-between text-sm">
                    <span className="font-medium text-gray-700 dark:text-gray-300">{indicator.name}</span>
                    <span className={cn(
                  ...
                  <div className="w-full h-2 bg-gray-200 dark:bg-gray-700 rounded-full overflow-hidden">
                    <div 
                      ...
                      style={{ width: `${getIndicatorPercent(indicator.value, indicator.max)}%` }}
                    />
                  </div>
                </div>
              ))}
